...
class Config:
    ...
    def __init__(self):
        self.data_path = self.get_path_from_env("RESUME_DATA_PATH", "./data/data.json", exists=True)
        self.disable_overwrites = self.get_boolean_from_env("RESUME_DISABLE_OVERWRITES", default=False)
        self.overwrites_path = self.get_path_from_env("RESUME_OVERWRITE_PATH", "./data/overwrites.json", exists=False)
        self.debug = self.get_boolean_from_env("RESUME_DEBUG", default=False)
    ...
    def get_path_from_env(var_name, default, is_dir=False, exists=True):
        ...
        Raises:
            ValueError: If the path does not exist or is not a directory when expected.
        """

        env_path = Path(os.getenv(var_name, default))
        if is_dir and not env_path.is_dir():
            raise ValueError(f"Expected directory at '{env_path}', but it does not exist or is not a directory.")

        if exists and not env_path.exists():
            raise ValueError(f"Expected path at '{env_path}', but it does not exist.")
